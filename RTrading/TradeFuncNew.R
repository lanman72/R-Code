...
#		sender <- "a@gmail.com"  # Replace with a valid address
#		recipients <- c("<a@gmail.com>")  # Replace with one or more valid addresses
#		email <- send.mail(from = sender,
#           		       to = recipients,
#		                   subject=subtxt,
#           		       body = btxt,
#		                   smtp = list(host.name = "smtp.gmail.com", port = 465, user.name = "@aol.com", passwd = "psw", ssl = TRUE),
#           		       authenticate = TRUE,
#		                   attach.files = attch,
#		                   send = TRUE,
#           		       debug = FALSE)
...
#		sender <- "a@gmail.com"  # Replace with a valid address
#		recipients <- c("<9991112222@vtext.com>")  # Replace with one or more valid addresses
#		email <- send.mail(from = sender,
#           		       to = recipients,
#		                   subject=subtxt,
#           		       body = btxt,
...
buildCharts <- function(indat, inasof, inhdr, n = 180) {
  #Assign date and Data from repor
  cdate <- paste("as of:", inasof)
  cdat <- as.table(indat[nrow(indat),])
  ...
  par(lwd = .75)
  plot(cd$Close, main=inhdr, ylim=yrange, ylab = "Price", xaxt="n")
  ...
  mtext(cdate, line=2, adj=.5, cex=.5, col="black")
